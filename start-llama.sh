...
# LOCAL_AI_LLAMA_DIR at wherever you put it. This script finds llama-server
# anywhere up to three levels inside that folder, because the tarballs differ
# in whether they have a top-level directory.
#
...
if [[ "$MODE" != "coder" && "$MODE" != "market" ]]; then
  echo "usage: $0 coder|market [options]   (see the header of this file)" >&2
  exit 2
fi
shift
...
MODEL_BYTES=$(stat -c %s "$MODEL_PATH" 2>/dev/null || stat -f %z "$MODEL_PATH")
MODEL_GB=$(awk -v b="$MODEL_BYTES" 'BEGIN{printf "%.2f", b/1073741824}')
say "model   : $MODEL_PATH ($MODEL_GB GB)"

# ---------------------------------------------------------------------------
# The binary. Tarballs differ in layout, so look a few levels down; fall back
# to one on PATH (a distro package or a source build).
# ---------------------------------------------------------------------------
SERVER=""
if [[ -d "$LLAMA_DIR" ]]; then
  SERVER=$(find "$LLAMA_DIR" -maxdepth 3 -type f -name llama-server -perm -u+x 2>/dev/null | head -1 || true)
fi
...
say "server  : $SERVER"
say "version : $("$SERVER" --version 2>&1 | head -1)"

# ---------------------------------------------------------------------------
# The GPU, and how much memory it has. Read it; never assume it.
# ---------------------------------------------------------------------------
GPU="${LOCAL_AI_GPU:-auto}"
if [[ "$GPU" == "auto" ]]; then
  if command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L >/dev/null 2>&1; then GPU=nvidia
  elif command -v rocm-smi >/dev/null 2>&1 || ls /sys/class/drm/card*/device/mem_info_vram_total >/dev/null 2>&1; then GPU=amd
  else GPU=cpu; fi
fi
...
if [[ -z "$VRAM_GB" && "$GPU" != "cpu" ]]; then
  case "$GPU" in
    nvidia)
      ...
      [[ -n "$mib" ]] && VRAM_GB=$(awk -v m="$mib" 'BEGIN{printf "%.0f", m/1024}') ;;
    amd)
      f=$(ls /sys/class/drm/card*/device/mem_info_vram_total 2>/dev/null | head -1 || true)
      if [[ -n "$f" ]]; then VRAM_GB=$(awk -v b="$(cat "$f")" 'BEGIN{printf "%.0f", b/1073741824}')
      elif command -v rocm-smi >/dev/null 2>&1; then
        b=$(rocm-smi --showmeminfo vram --csv 2>/dev/null | awk -F, 'NR==2{print $2}' || true)
        [[ -n "$b" ]] && VRAM_GB=$(awk -v b="$b" 'BEGIN{printf "%.0f", b/1073741824}')
      fi ;;
  esac
fi
if [[ "$GPU" != "cpu" && -z "$VRAM_GB" ]]; then
...
say "gpu     : $GPU${VRAM_GB:+ ($VRAM_GB GB)}"

# ---------------------------------------------------------------------------
# Threads: physical cores, like the Windows launchers' -t 16 on a 16-core part.
# ---------------------------------------------------------------------------
if [[ -z "$THREADS" ]]; then
  THREADS=$(lscpu -p=core 2>/dev/null | grep -v '^#' | sort -u | wc -l | tr -d ' ')
  [[ "${THREADS:-0}" -ge 1 ]] || THREADS=$(( $(nproc 2>/dev/null || echo 4) / 2 ))
  [[ "$THREADS" -ge 1 ]] || THREADS=1
fi

# ---------------------------------------------------------------------------
# The VRAM budget — the same arithmetic as the .ps1 files, in awk.
# ---------------------------------------------------------------------------
KV_GB=$(awk -v k="$KV_KIB_PER_TOKEN" -v c="$CTX" 'BEGIN{printf "%.2f", k*c/1048576}')
NGL=0
if [[ "$GPU" == "cpu" ]]; then
  say "budget  : no GPU — everything on the CPU (slow; --cpu-moe irrelevant)"
  CPU_MOE=0; N_CPU_MOE=0
else
  USABLE=$(awk -v v="$VRAM_GB" -v r="$RESERVE_GB" 'BEGIN{printf "%.2f", v-r}')
  BUDGET=$(awk -v u="$USABLE" -v k="$KV_GB" 'BEGIN{printf "%.2f", u-k}')
  ...
  if awk -v b="$BUDGET" 'BEGIN{exit !(b<=0)}'; then
    die "--ctx $CTX needs $KV_GB GB of KV cache, which does not fit in $USABLE GB on its own. Lower --ctx."
  fi
  if [[ "$NGL_FORCE" -gt 0 ]]; then
    NGL=$NGL_FORCE
    FITS=$(awk -v L="$LAYERS" -v b="$BUDGET" -v m="$MODEL_GB" 'BEGIN{printf "%d", L*b/m}')
    ...
    NGL=$(awk -v L="$LAYERS" -v b="$BUDGET" -v m="$MODEL_GB" 'BEGIN{n=int(L*b/m); if(n>L)n=L; if(n<0)n=0; print n}')
  fi
  if [[ "$CPU_MOE" == "1" || "$N_CPU_MOE" -gt 0 ]]; then
    ...
    ON_GPU=$(awk -v m="$MODEL_GB" -v n="$NGL" -v L="$LAYERS" 'BEGIN{printf "%.2f", m*n/L}')
...
# do not know --load-mode. Ask the binary, once, with no model loaded.
# ---------------------------------------------------------------------------
if "$SERVER" --help 2>&1 | grep -q -- '--load-mode'; then LOAD_FLAG=(--load-mode none); else LOAD_FLAG=(--no-mmap); fi
...
# ---------------------------------------------------------------------------
# Stop a previous server and wait for the card to give its memory back.
# Killing one that holds 14 GB and allocating 14 GB a second later is what hung
# the reference machine; the wait here mirrors the Windows launcher.
# ---------------------------------------------------------------------------
  ...
  case "$GPU" in
    nvidia)
      for _ in $(seq 1 45); do
        ...
        [[ "${used:-0}" -le 2048 ]] && break; sleep 1
      done
      [[ "${used:-0}" -le 2048 ]] || warn "$((used/1024)) GB of VRAM still held after 45s; starting anyway" ;;
    amd)
      f=$(ls /sys/class/drm/card*/device/mem_info_vram_used 2>/dev/null | head -1 || true)
      if [[ -n "$f" ]]; then
        for _ in $(seq 1 45); do used=$(cat "$f"); [[ "$used" -le 2147483648 ]] && break; sleep 1; done
...
# ---------------------------------------------------------------------------
# Start, and wait until it actually answers.
# ---------------------------------------------------------------------------
...
for i in $(seq 1 120); do
  if [[ "$(curl -s -o /dev/null -w '%{http_code}' --max-time 3 "http://$BIND:$PORT/health" 2>/dev/null)" == "200" ]]; then
    say "READY   : http://$BIND:$PORT  ($ALIAS)"
    exit 0
  fi
